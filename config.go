...
type Config struct {
	...
	// TitleWithKeyword 决定浏览器标签页标题是否带上当前搜索关键词。
	//
	// 关闭（**默认**）时标题恒为「资源搜索」；开启后是「关键词 · 资源搜索」。
	// 默认关闭是刻意的：标签标题会留在浏览器历史与书签里，也会被旁边的人
	// 一眼看到，默认不把"正在搜什么"摆在明面上；要露的用户自己打开。
	//
	// 这里用 omitempty 是刻意的——**文件里只记录偏离默认值的选择**。
	// 它是一道提前埋好的保险：哪天要把默认值改成"带关键词"，
	// 存量 config.json 不会像 max_results 那样把旧默认值钉死
	// （那个坑见 MaxResults 的注释）。代价是"显式关闭"与"从未设置"
	// 在文件里长得一样，而这两者当前行为完全相同，所以不构成损失。
	//
	// 与 PageSize 的处理不同也是刻意的：PageSize 的 0 是一个合法取值，
	// 一旦省略就无法区分"全部"与"没配"，因此它必须照写。
	TitleWithKeyword bool `json:"title_with_keyword,omitempty"`

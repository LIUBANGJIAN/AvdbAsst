...
| 下载器 | 提交离线下载用的下载器标识，**留空即继承上游全局设置**（推荐） |
| 保存路径 | 提交离线下载时的目录，留空即继承上游全局保存目录（推荐） |
...
访问令牌（`X-API-Key`）无权调用。因此本站无法用你填的令牌自动列出下载器供选择。

好在**不填就是最正确的填法**：上游把 `downloader` / `save_path` 的空值解释为
「使用服务端全局配置」，所以两项都留空即可，不需要手输、也不会输错。
真填错了，报错会照实显示上游的校验详情（例如
`上游参数校验失败（422）：查询参数 → save_path：Field required`），便于对照修正。
...
| `AVDB_DOWNLOADER` | 空 | 下载器标识，留空即继承上游全局设置 |
| `AVDB_SAVE_PATH` | 空 | 下载保存路径，留空即继承上游全局保存目录 |
...
- `上游参数校验失败（422）：查询参数 → downloader：...`
  你手填的下载器标识上游不认。**把「下载器」与「保存路径」都留空**，
  让上游用自己的全局配置，这是最不容易出错的用法。
- `下载器未配置` / 类似的业务错误码

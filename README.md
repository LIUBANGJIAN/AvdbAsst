...
- 结果**分页展示**，每页条数可选 10/50/100/200/300/500/1000 或「全部」，设置一次**永久记住**
...
| 下载器 | 提交离线下载用的下载器标识，**必填**。不知道填什么就点「探测可用下载器」，见下节 |
| 保存路径 | 提交离线下载的目录，**必填且不能为空**（上游会直接拒绝空目录） |
| 每页条数 | 结果页每页显示多少条，设置页与结果页两处都能改，**保存后永久生效** |
| 访问口令 | 给本站自己加的访问控制，见下节 |

「请求超时」与「结果上限」两个高级项已下线：超时改为固定上限，
结果上限降级为内部安全阀，两者都不再需要用户关心。

点「保存设置」即时生效，**不需要重启容器**。

页面上有四个探测按钮，彼此独立，任一失败不影响其余：

| 按钮 | 打到上游的接口 | 用途 |
| --- | --- | --- |
| 测试连接 | `GET /api/v1/articles/torrents` | 保存前确认地址和令牌能不能用 |
| 探测可用下载器 | 对每个候选标识各调一次 `GET /api/v1/config/downloader/directories` | **最有用**：反推出上游实际配了哪个下载器，并直接回填 |
| 读取上游默认下载器 | `GET /api/v1/javdb/subscriptions/default-rule` | 把上游订阅规则里的下载器 + 目录取回来填进输入框 |
| 校验下载器并列出目录 | `GET /api/v1/config/downloader/directories` | 确认某个标识有效，并带回它下面的目录做下拉提示 |

四个接口的鉴权都是 `API Key/JWT`，**只需访问令牌，不需要上游账号密码**。
...
- 设置页「探测可用下载器」对 `clouddrive / 115 / qbittorrent / transmission / thunder`
  各探一次，把**命中 configured 的第一个**作为推荐值回填；
...
| `AVDB_MAX_RESULTS` | `5000` | 内部安全阀（1–50000），**不是结果上限**：搜索本身无上限，超出的部分照常分页展示 |
...
- `truncated` 为 `true` 表示命中数超过内部安全阀（默认 5000）被截断——
  属于异常情况，正常搜索无结果上限。
...
- `上游参数校验失败（422）：查询参数 → downloader：Field required`
  ...
  把提示发出来即可定位。
- `上游没有找到下载器 "xxx"`
  你在设置页写死的标识上游不认。点「探测可用下载器」拿回真实标识再保存。
...
│       └── settings.js          测试连接、探测可用下载器、读取上游默认下载器、校验下载器
